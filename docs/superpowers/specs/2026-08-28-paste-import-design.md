...
- Erkennung: Few-Shot-Beispiele DE/EN (Flug inkl. LCC/Screenshot/PNR-unter-Label, Bahn DB/ÖBB/Trainline, Bus/FlixBus/TGV→`train`, Hotel Booking.com/Airbnb, Mietwagen Sixt/Hertz, Event GetYourGuide/Eventbrite, Fähre) in `PasteImportExtractionInstructions`; Typ-Aliase und Ticket-Datenformate im Mapper; Titel-/Operator-Hinweise (`PasteImportExtractionTypeHint`) wenn das Modell den Typ weglässt; AGB-Seiten im PDF-Text weglassen; Material-Zeichenbudget (`PasteImportPromptBudget`) vor dem Modellaufruf; `confirmationCode` ohne Labels/Preise/Initialen; fehlendes `startAt` aus Abfahrt/Check-in/Itinerar-Routenzeilen im Quelltext (eine Buchung oder 1:1 bei gleicher Anzahl); komplementäre Bordkarten-Fragmente zusammenführen (`PasteImportExtractionCoalescer`); Extrakte ohne Beleg im Quelltext verwerfen (`PasteImportSourceGrounding`); Boarding-Pass ohne mehrere Abflüge = ein Eintrag. Unbekanntes Typ-Label bleibt `nil`. Gescannte Bilder nur, wenn die Runtime `Attachment` exportiert — sonst `imageInputUnsupported`, kein Crash.
