...
public enum SelectionBatchDeleteHandlers {
    ...
    public static func deleteOpenBookings(
        ...
        run(ids: ids, started: SelectionBatchDeleteDiagnostics.openBookingList(result: .started, count: ids.count)) { count, errorType in
            if let errorType {
                SelectionBatchDeleteDiagnostics.openBookingList(result: .failed, count: count, errorType: errorType)
            } else {
    ...
    public static func deleteTripBookings(
        ...
        run(ids: ids, started: SelectionBatchDeleteDiagnostics.tripBookingList(result: .started, count: ids.count)) { count, errorType in
            if let errorType {
                SelectionBatchDeleteDiagnostics.tripBookingList(result: .failed, count: count, errorType: errorType)
            } else {
    ...
    public static func deleteTrips(
        ...
        run(ids: ids, started: SelectionBatchDeleteDiagnostics.tripList(result: .started, count: ids.count)) { count, errorType in
            if let errorType {
                SelectionBatchDeleteDiagnostics.tripList(result: .failed, count: count, errorType: errorType)
            } else {
    ...
    private static func run<ID: Hashable>(
        ...
        case .failed:
            events.append(finish(ids.count, "Error"))
        }
