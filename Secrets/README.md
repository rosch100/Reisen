...
Gmail-Feedback-Ingress: OAuth-Client und Refresh-Token nur als GitHub-Actions-Secrets (`REISEN_GMAIL_OAUTH_*`) — siehe `docs/ci/gmail-feedback-ingress.md`. Der Grok-Bot-Rechner braucht dieselben Werte unter den Namen `ISSUE_DEV_GMAIL_OAUTH_*` plus `ISSUE_DEV_GMAIL_EXPECTED_ADDRESS`. Nicht in diesem Verzeichnis ablegen.
