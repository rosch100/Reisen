...
if [[ "$ALTOOL_STATUS" -ne 0 ]]; then
  if grep -q "Unable to find Apple ID for Bundle ID" "$JSON_OUT"; then
    echo "Fehler: In App Store Connect fehlt eine iOS-App für Bundle-ID de.reisen.Reisen.ios." >&2
    echo "Developer-Portal-App-ID reicht nicht. Unter https://appstoreconnect.apple.com/apps eine neue iOS-App anlegen (Bundle-ID de.reisen.Reisen.ios)." >&2
