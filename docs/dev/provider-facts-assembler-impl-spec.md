...
`bookingType(of:)` bleibt Provider: `"Flight"` / `"ACCOMMODATION"` / Traveloka-Product-Strings sind kein Domain-Vokabular. Katalog- und Detail-Mapper (Airbnb, GYG, Opodo, Booking.com, Traveloka, Check24, billiger-mietwagen) rufen nur `DraftAssembler` — kein paralleles `ProviderBookingDraft(`- oder `ProviderBookingEnrichment(`-Init.
